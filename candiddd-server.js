#!/usr/bin/env node
// ═══════════════════════════════════════════════════════════════
// CANDIDDD.live — Backend v2
// Fix: sicurezza, CORS, broadcast, package.json main
// ═══════════════════════════════════════════════════════════════
...
// ─────────────────────────────────────────────────────────────
// CONFIG — crasha se le variabili critiche mancano
// ─────────────────────────────────────────────────────────────
function requireEnv(name) {
  const val = process.env[name];
  if (!val) {
    console.error(`ERRORE: variabile d'ambiente ${name} mancante. Aggiungila su Render.`);
    process.exit(1);
  }
  return val;
}
...
function emitToSet(set, evt) {
  for (const socket of set) wsEmit(socket, evt);
}

function emitToViewers(sessionId, evt) { emitToSet(getRoom(sessionId).viewers, evt); }
function emitToAdmins(sessionId, evt)  { emitToSet(getRoom(sessionId).admins, evt); }
function emitToAll(sessionId, evt)     { emitToViewers(sessionId, evt); emitToAdmins(sessionId, evt); }
...
function getCorsHeaders(origin) {
  ...
  const isAllowed = allowed.includes(origin) || (origin && origin.endsWith('.candiddd.pages.dev'));
  return {
...
const server = http.createServer(async (req, res) => {
  ...
  if (method === 'GET' && path === '/api/health') {
    return respond(res, req, {
      ...
      config: { livekit_url: CONFIG.LIVEKIT_URL, worker_url: CONFIG.WORKER_URL }
    });
  }
  ...
  if (method === 'POST' && path === '/api/creator/live-sessions') {
    ...
    if (!validLevels.includes(level)) return respond(res, req, { error: 'Level non valido' }, 400);
    ...
    catch (e) { return respond(res, req, { error: 'Errore token LiveKit: ' + e.message }, 500); }

    const session = {
      id, creator_id: creatorId, status: 'LIVE', level, mission_id,
      ...
      start_time: now, end_time: null, viewer_count: 0, earnings_eur: 0, created_at: now,
    };
    db.sessions.set(id, session);
    emitToAll(id, { type: 'live_started', payload: { session_id: id, creator_id: creatorId, start_time: now, level } });
    console.log(`[SESSION] Created: ${id} by ${creator_name || creatorId}`);

    return respond(res, req, {
      live_session_id: id, livekit_token: livekitToken,
      livekit_url: CONFIG.LIVEKIT_URL,
      stream_key: session.stream_key, playback_url: session.playback_url,
      hls_url: session.hls_url, status: 'LIVE', level,
    }, 201);
  }
  ...
  if (method === 'PATCH' && endMatch) {
    ...
    if (!session) return respond(res, req, { error: 'Sessione non trovata' }, 404);
    if (session.creator_id !== creatorId) return respond(res, req, { error: 'Non autorizzato' }, 403);
    if (session.status === 'ENDED') return respond(res, req, { session_id: endMatch[1], status: 'ENDED', already_ended: true });
    const now = new Date().toISOString();
    ...
    return respond(res, req, { live_session_id: endMatch[1], status: 'ENDED', end_time: now, duration_seconds: durationSec });
  }
  ...
  if (method === 'GET' && viewersMatch) {
    ...
    try {
      ...
      return respond(res, req, { viewer_count: count, session_id: viewersMatch[1] });
    } catch {
      return respond(res, req, { viewer_count: session.viewer_count || 0 });
    }
  }
  ...
  if (method === 'GET' && sessionMatch) {
    ...
    return respond(res, req, {
      ...
      playback_url: session.playback_url, hls_url: session.hls_url,
      ...
      mission: null, viewer_count: session.viewer_count,
  ...
  if (method === 'GET' && path === '/api/live-sessions') {
    const active = [...db.sessions.values()]
      ...
      .map(s => ({
        ...
        creator_city: s.creator_city || '',
        level: s.level, playback_url: s.playback_url,
        viewer_count: s.viewer_count, start_time: s.start_time,
      }));
    return respond(res, req, { live_sessions: active, total: active.length });
  }
  ...
  if (method === 'POST' && donMatch) {
    ...
    if (!session || session.status !== 'LIVE') return respond(res, req, { error: 'Live non attiva' }, 400);
    const amount = parseFloat(body.amount);
    const fraud = fraudCheck(userId, amount);
    if (!fraud.ok) return respond(res, req, { error: fraud.message, reason: fraud.reason }, 400);
    const don = {
      id: crypto.randomUUID(), live_session_id: donMatch[1], user_id: userId,
      from: String(body.from || 'Anonimo').substring(0, 30), amount,
      message: String(body.message || '').substring(0, 100),
      flagged: fraud.flagged, flag_reason: fraud.flag_reason,
    ...
    return respond(res, req, { ok: true, donation_id: don.id, flagged: fraud.flagged }, 201);
  }
  ...
  if (method === 'POST' && repMatch) {
    ...
    if (!session) return respond(res, req, { error: 'Sessione non trovata' }, 404);
    const report = {
      id: crypto.randomUUID(), live_session_id: repMatch[1], viewer_id: viewerId,
      reason: String(body.reason || '').substring(0, 200),
      resolution: 'PENDING', created_at: new Date().toISOString(),
    };
    ...
    return respond(res, req, { ok: true, report_id: report.id, total_reports: total });
  }

  // ── POST /api/admin/live-sessions/:id/actions ────────────
  const modMatch = path.match(/^\/api\/admin\/live-sessions\/([^/]+)\/actions$/);
  if (method === 'POST' && modMatch) {
    if (!isAdmin) return respond(res, req, { error: 'Non autorizzato' }, 401);
    ...
    if (!allowed.includes(body.action)) return respond(res, req, { error: 'Azione non valida', allowed }, 400);
    ...
    return respond(res, req, { ok: true, action: body.action, log_id: log.id });
  }
  ...
  if (method === 'GET' && path === '/api/admin/live-sessions') {
    ...
    return respond(res, req, { sessions: all, total: all.length });
  }

  respond(res, req, { error: 'Not found', path }, 404);
});
...
server.on('upgrade', (req, socket) => {
  ...
  const acceptKey = require('crypto').createHash('sha1')
  ...
  socket.on('data', (buffer) => {
    try {
      ...
      if (data.type === 'chat_message') {
        emitToAll(sessionId, { type: 'chat_message', payload: { from: String(data.payload?.from || 'Anonimo').substring(0,30), message: String(data.payload?.message || '').substring(0,200), ts: new Date().toISOString() } });
      }
    } catch {}
  });
  ...
  socket.on('close', () => {
    ...
    const session = db.sessions.get(sessionId);
    if (session && socket.role === 'viewer') session.viewer_count = room.viewers.size;
  });
  socket.on('error', () => socket.destroy());
});

// ─────────────────────────────────────────────────────────────
// PULIZIA AUTOMATICA SESSIONI — ogni 5 minuti
// ─────────────────────────────────────────────────────────────
async function cleanupSessions() {
  const liveSessions = [...db.sessions.values()].filter(s => s.status === 'LIVE');
  if (liveSessions.length === 0) return;
  console.log(`[CLEANUP] Controllo ${liveSessions.length} sessioni...`);
  try {
    const svc = new RoomServiceClient(
      CONFIG.LIVEKIT_URL.replace('wss://', 'https://'),
      CONFIG.LIVEKIT_API_KEY, CONFIG.LIVEKIT_API_SECRET,
    ...
    for (const session of liveSessions) {
      ...
      if (!activeRoomNames.has(session.id) || participants === 0) {
        session.status = 'ENDED';
        session.end_time = new Date().toISOString();
        console.log(`[CLEANUP] Sessione ${session.id.substring(0,8)} chiusa`);
      } else {
...
server.listen(CONFIG.PORT, '0.0.0.0', () => {
  console.log(`\n🚀 CANDIDDD Backend v2 — porta ${CONFIG.PORT}`);

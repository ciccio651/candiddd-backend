...
const server = http.createServer(async (req, res) => {
  ...
  if (method === 'GET' && viewersMatch) {
    ...
    try {
      ...
      const count = room ? Math.max(0, (room.numParticipants || 0) - 1) : 0; // -1 per escludere il creator
      session.viewer_count = count;
      return respond(res, { viewer_count: count, session_id: viewersMatch[1] });
    } catch (e) {
      return respond(res, { viewer_count: session.viewer_count || 0 });
    }
  }
